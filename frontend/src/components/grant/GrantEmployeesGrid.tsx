...
interface GrantEmployeeGridProps {
  //openSubpage: (grantData: any) => void;
  grantEmployeeData: GrantEmployeeGridRow[];
}

const GrantEmployeeGrid: React.FC<GrantEmployeeGridProps> = ({ grantEmployeeData }) => {

  /*

  const findItemById = (id: number) => {
    return grantEmployeeData.find(dataItem => dataItem.employeeId === id);
  };
  ...
  */
  ...
  const rows: GridRowsProp = grantEmployeeData.map((row: GrantEmployeeGridRow) => {
    return {
      uID: row.uID,
...
export default GrantEmployeeGrid;
