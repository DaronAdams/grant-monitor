...
const Visuals: React.FC<GrantMainTabProps> = ({ grantData }) => {
  ...
  return (
    <>
      ...
      <div className="flex flex-col mx-auto justify-center items-center" ref={pdfRef}>
        <div className="flex flex-row justify-between items-center p-4">
          <GrantBarChart startDate={new Date(grantData.startDate)} endDate={new Date(grantData.endDate)} TotalAmount={grantData.totalAmount} transactions={grantTransactionsData} />
        </div>

        <div className="mx-1"></div>

        <div className="flex flex-row justify-between items-center p-4">
          <GrantLineChart startDate={new Date(grantData.startDate)} endDate={new Date(grantData.endDate)} TotalAmount={grantData.totalAmount} transactions={grantTransactionsData} />
        </div>
      </div>
